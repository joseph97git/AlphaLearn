...
# reshape into 3D
n_features <- 1
dimTrain1 <- dim(train)[1]
dimTrain2 <- dim(train)[2]
dim(train) <- c(dimTrain1, dimTrain2, n_features)
...
model <- keras_model_sequential() 
model%>%
  layer_lstm(units, batch_input_shape = c(batch_size, dimTrain1, dimTrain2), stateful= TRUE)%>%
  layer_dense(units = 1)
```
